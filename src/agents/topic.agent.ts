...
import { AgentOutputInvalidError, TopicNotUniqueError } from '../types/errors/agent.error.js';
...
export class TopicAgent implements Agent<TopicRequestDto, TopicDto> {
  ...
  private async findUniqueTopic(input: TopicRequestDto, logger: Logger): Promise<TopicDto> {
    ...
    for (let attempt = 1; attempt <= this.config.topicMaxAttempts; attempt += 1) {
      const candidate = await this.proposeCandidate(input, excluded);
    ...
    throw new TopicNotUniqueError(this.config.topicMaxAttempts, rejected);
  }
