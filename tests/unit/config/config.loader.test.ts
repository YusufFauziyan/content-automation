...
const VALID_ENV: RawEnvironment = {
  ...
  KOKORO_BASE_URL: 'http://kokoro.test/v1',
  KOKORO_MODEL: 'kokoro',
...
describe('loadConfig', () => {
  ...
  it('runs the whole implemented pipeline by default', () => {
    ...
    expect(config.pipeline.lastStep).toBe(WorkflowStepName.Compose);
  });

  it('exposes the speech settings without a hard-coded voice', () => {
    const config = loadConfig({ ...VALID_ENV, KOKORO_VOICE: 'af_bella', KOKORO_SPEED: '1.2' });

    expect(config.kokoro.voice).toBe('af_bella');
    expect(config.kokoro.speed).toBe(1.2);
    expect(config.kokoro.baseUrl).toBe('http://kokoro.test/v1');
  });

  it('rejects a missing speech endpoint', () => {
    const { KOKORO_BASE_URL: _url, ...withoutUrl } = VALID_ENV;

    expect(() => loadConfig(withoutUrl)).toThrow(ConfigurationError);
  });
  ...
  it('refuses a step that is declared but not implemented', () => {
    expect(() => loadConfig({ ...VALID_ENV, PIPELINE_LAST_STEP: 'UPLOAD' })).toThrow(
      ConfigurationError,
    );
  });
