...
describe('buildFilterGraph', () => {
  ...
  it('positions the subtitles at the bottom, centred', () => {
    ...
    expect(SUBTITLE_STYLE).toContain('Alignment=2');
    expect(SUBTITLE_STYLE).toMatch(/MarginV=\d+/u);
  });
  ...
  it('keeps a cue to the lines the subtitle agent chose', () => {
    ...
    expect(SUBTITLE_STYLE).toContain('WrapStyle=2');
  });
  ...
  it('sizes the caption as a fraction of the frame, not in pixels', () => {
    ...
    const size = /FontSize=(\d+)/u.exec(SUBTITLE_STYLE)?.[1];
