...
const ffmpegReturning = (
  ...
  failure?: Error,
): { service: FfmpegService; outputs: string[]; covers: string[] } => {
  ...
  return {
    ...
    service: {
      ...
      makeCover: (imagePath, outputPath) => {
        covers.push(`${imagePath} -> ${outputPath}`);
...
/** The composer's own settings; only the cover fraction matters here. */
const videoConfig: VideoConfig = asFake<VideoConfig>({
  width: 1080,
  height: 1920,
