...
import type {
  AudioRepository,
  NarrationAudioDto,
} from '../../../src/repositories/audio.repository.js';
...
import type { SpeechRequest, SpeechService } from '../../../src/services/speech.service.js';
import {
  WorkspaceFolder,
  type StoredFileDto,
...
const speechReturning = (requests: SpeechRequest[], failing = false): SpeechService => ({
...
/** The audio metadata write is not what these tests are about. */
const audioRepository = asFake<AudioRepository>({
  save: () => Promise.resolve(asFake<NarrationAudioDto>({ id: 'audio-1' })),
});
