 ...
 *
 * Dependencies
 * - `SpeechService` — synthesises each block.
...
export class VoiceAgent implements Agent<VoiceRequestDto, VoiceGenerationResponseDto> {
  ...
  public async execute(input: VoiceRequestDto): Promise<Result<VoiceGenerationResponseDto>> {
    ...
    try {
      ...
      const clips = await this.speakBlocks(spoken, workspace, input.language, logger);
      ...
      // Recorded for the same reason the stills and the render are: a resumed
      // run skips this step because it succeeded, and would otherwise have
      // nothing to hand the renderer. The bytes stay on disk and are still
      // deleted after publishing; only what they are survives.
      await this.audioRepository.save({
        ...
        durationMs: Math.round(audio.durationSeconds * MILLISECONDS_PER_SECOND),
        voice: audio.voice,
        model: audio.model,
        speed: audio.speed,
        generationDurationMs,
  ...
  private async speakBlocks(
    ...
    for (const block of blocks) {
      const spoken = await this.speech.synthesize({ text: block.text, language });
