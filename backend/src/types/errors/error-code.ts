...
export enum ErrorCode {
  ...
  /** A step was left mid-flight by a process that stopped. Resumable. */
  ...
  // Deprecated: raised only by the Hugging Face image fallback, which was
  // removed. Kept because `workflow_step_runs.last_error` rows still carry
  // them, and a code is a contract even after nothing throws it.
  ...
  /** No enabled credential exists for the platform a run wants to publish to. */
  CredentialMissing = 'CREDENTIAL_MISSING',
  /** The captured browser session is no longer signed in. Log in again. */
  PublishSessionExpired = 'PUBLISH_SESSION_EXPIRED',
  /** The platform refused the upload, or the page never reached a post state. */
  PublishFailed = 'PUBLISH_FAILED',
  /** A publish step exceeded its configured time budget. */
  PublishTimeout = 'PUBLISH_TIMEOUT',
  /** The upload was submitted but the published video could not be confirmed. */
  PublishNotVerified = 'PUBLISH_NOT_VERIFIED',
