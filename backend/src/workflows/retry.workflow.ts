...
export class RetryWorkflow {
  ...
  /**
   * Resumes one named run.
   *
   * Separate from the sweep because the reasons differ: the sweep is a
   * scheduled clean-up of a backlog, this is a person looking at a run that
   * failed and deciding to continue it now.
   *
   * @param correlationId Identity of the run to continue.
  ...
  public async resumeRun(
    ...
    const logger = this.logger.child({ source: RetryWorkflow.name, correlationId });
    logger.info('Resume requested for a single run');
    ...
    if (!result.success) {
      logger.warn('Resume attempt failed', { errorCode: result.error.code });
      return { inspected: 1, resumed: 0, failed: 1 };
    }

    return { inspected: 1, resumed: 1, failed: 0 };
  }
