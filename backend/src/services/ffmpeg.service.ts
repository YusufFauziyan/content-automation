...
export interface FfmpegService {
  ...
  /**
   * Writes `imagePath` out as a cover at the configured frame size.
   *
   * @throws {RenderError} When FFmpeg cannot read or write the image.
   */
  makeCover(imagePath: string, outputPath: string): Promise<void>;
}
 ...
 *
 * Takes the height rather than reading a constant: where the captions belong
 * depends on what draws over them, and that is a deployment's decision. See
 * `VIDEO_SUBTITLE_BOTTOM_FRACTION`.
 */
export const toSubtitleStyle = (bottomFraction: number): string =>
  [
    'Alignment=2',
    'WrapStyle=2',
    ...
    'Outline=1',
    'Shadow=0',
    `MarginV=${String(toAssUnits(bottomFraction))}`,
 ...
 */
export const buildFilterGraph = (plan: RenderPlanDto, subtitleBottomFraction: number): string => {
  ...
  const style = toSubtitleStyle(subtitleBottomFraction);
  const burn = `[vcat]subtitles='${escapeFilterPath(plan.subtitlePath)}':force_style='${style}'[vout]`;
...
export const buildRenderArguments = (
  ...
  const graph = `${buildFilterGraph(plan, config.subtitleBottomFraction)};${buildAudioGraph(plan, narrationIndex)}`;
 ...
 * that a 24 kHz speech engine is upsampled rather than a higher-rate one
 * being degraded.
 */
...
export class ProcessFfmpegService implements FfmpegService {
  ...
  /**
   * Writes a still out as a cover at the video's own frame size.
   *
   * Scaled to cover and then cropped rather than letterboxed: a cover with bars
   * down the sides is the thing every platform crops badly anyway, and the
   * stills are already the right shape, so the crop takes almost nothing.
   *
   * Quality 2 is near the top of the JPEG scale. This file is the first thing
   * anybody sees of the video and is a few hundred kilobytes at most.
   */
  public async makeCover(imagePath: string, outputPath: string): Promise<void> {
