...
export interface WorkingDirectoryService {
  ...
  /**
   * File names present in a sub-directory, sorted.
   *
   * A missing directory reads as empty rather than as a failure: asking what is
   * in a folder that was never created is a fair question with a fair answer.
   */
  list(workspace: WorkspaceDto, folder: WorkspaceFolder): Promise<readonly string[]>;

  /** Reads back the bytes of a file in the workspace. */
  read(workspace: WorkspaceDto, folder: WorkspaceFolder, fileName: string): Promise<Uint8Array>;
...
export class LocalWorkingDirectoryService implements WorkingDirectoryService {
  ...
  public async list(workspace: WorkspaceDto, folder: WorkspaceFolder): Promise<readonly string[]> {
    try {
      const names = await readdir(workspace.folders[folder]);
      return names.sort();
    } catch {
      return [];
    }
