...
export class WorkflowRepository {
  ...
  /**
   * Every step left mid-flight, whatever its run's status says.
   *
   * Asked for directly rather than by walking runs: a `RUNNING` row is the
   * authority on "something is executing", and a run can perfectly well look
   * finished while carrying one. That combination is what left the editor
   * spinning on a step nothing was working on.
   */
  public async findStrandedSteps(limit: number): Promise<readonly WorkflowStepRunDto[]> {
    const records = await runQuery('WorkflowRepository.findStrandedSteps', () =>
      this.database.workflowStepRun.findMany({
        where: { status: toDbWorkflowStepStatus(WorkflowStepStatus.Running) },
   ...
   *
   * Deliberately the opposite order to {@link findResumableRuns}: a person
   * looking at a list wants what just happened, while the resumer works through
   * the oldest backlog first.
   */
  public async findRecentRuns(limit: number): Promise<readonly WorkflowRunDto[]> {
    const records = await runQuery('WorkflowRepository.findRecentRuns', () =>
      this.database.workflowRun.findMany({
  ...
  /**
   * Removes runs by id and reports how many rows went.
   *
   * Step rows and log records cascade with the run. The content it produced —
   * topic, script, scene plan — does not: those columns are `SetNull`, because
   * the knowledge outlives the execution that made it (CLAUDE.md "Core
   * Philosophy").
   */
  public async deleteRuns(ids: readonly string[]): Promise<number> {
    if (ids.length === 0) return 0;

    const { count } = await runQuery('WorkflowRepository.deleteRuns', () =>
      this.database.workflowRun.deleteMany({ where: { id: { in: [...ids] } } }),
    );
