...
export class LogRepository {
  ...
  /**
   * The newest records across every run.
   *
   * Ordered newest first because a log is read from the bottom of the incident
   * backwards, not from the beginning of time forwards.
   */
  public async findRecent(limit: number): Promise<readonly LogEntryDto[]> {
    const records = await runQuery('LogRepository.findRecent', () =>
      this.database.logEntry.findMany({
