...
export interface TopicRequestDto {
  ...
  /**
   * A title supplied by a person, used instead of asking a model for one.
   *
   * It still goes through duplicate detection: a human can propose the same
   * subject twice as easily as a model can, and the whole point of that check
   * is that no two videos cover the same ground.
   */
